...
Router.map(function() {
  this.route('datasets', function() {
    ...
    // this.mount('edit-engine', { path: '/:dataset_id/edit' }); // if we do it this way we get params.dataset_id, but we do not get the resolved model, and we are not in our datasets.dataset outlet
  });
  this.route('documents', function() {
    this.route('document', { path: '/:document_id' }, function () {
      this.mount('edit-engine', { path: '/edit' });
    });
